...
class StaffController extends Controller
{
    ...
    public function actionIndex()
    {
        ...
        if (Yii::$app->request->post('hasEditable')) {
            ...
            $staff_id = Yii::$app->request->post('editableKey');
            $model = Staff::findOne($staff_id);

            if(!$model) {
                // store a default json response as desired by editable
                $message = Yii::t('app', 'The Staff do not exist.');
                echo $out = Json::encode(['output'=>'', 'message'=>$message]);
                return;
            }

            $post = [];
            $posted = current($_POST['Staff']);
            $post['Staff'] = $posted;
            ...
            try{
                ...
                if($address->save()) {
                    ...
                    if ($model->save()) {
                        ...
                        $dir = Yii::getAlias('@backend/web/uploads/staffs/image/' . $model->id);
                        ...
                        Yii::$app->getSession()->setFlash('success', [
                            ...
                            'duration' => 3000,
                            'icon' => 'fa fa-plus',
                            'message' => Yii::t('app', 'Staff Record has been saved.'),
                            'title' => Yii::t('app', 'Add Staff'),
                        ]);
...
//                        $model->re_password = $re_password_return;
    ...
    public function actionUpdate($id)
    {
        ...
        if ($model->load(Yii::$app->request->post())
            ...
            if (Yii::$app->request->post('Staff')['status'] === '1') {
                $model->status = 1;
            } else {
                $model->status = 0;
            }

            if(Yii::$app->request->post('Staff')['password'] === '') {
